...
const Statistics = ({repoId}) =>{
  ...
  useEffect(()=>{
    ...
    console.log("fetchingRepo")
  },[])

  const classes = useStyles();
  if(typeof repository.data.full_name != "undefined")
      return(
        <>
        {(typeof repository === "undefined" && "undefined")||
           (repository.loading && "Loading...") ||
          (repository.error && repository.error.toString())||
          <div style={{ height: '600px',display:'inline-table',width:"100%",marginLeft:'2%'}}>
                <div className={classes.Pie}>
                       <span className={classes.Header1}>The Impact of each user on the project given the last 100 commits </span>
                       <Pie full_name = {repository.data.full_name} />
                </div>
                <div className={classes.Timeline}>
                       <span className={classes.Header2}>The projection of the last 100 commits on a timeline </span>
                       <div className={classes.TimelinePadding}>
                         <TimeLineStream full_name = {repository.data.full_name} />
                       </div>
                </div>
          </div>
        }
...
//
//
//<TimeLineStream full_name = {repository.data.full_name} style={{width:'inherit'}}/>
//
//   render(){
...
const useStyles = makeStyles({
 ...
 Header2 : { fontFamily: "Calibri",fontSize:28,marginTop:'2%', fontStyle: "normal", fontVariant: "normal",marginLeft:'1%',padding:'1%' }
